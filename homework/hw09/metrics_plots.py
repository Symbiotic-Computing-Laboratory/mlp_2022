...
def ks_roc_prc_plot(targets, scores, FIGWIDTH=15, FIGHEIGHT=6, FONTSIZE=14):
    ...
    thresholds_prc = np.append(thresholds_prc, 1.0)
    ...
    # Generate KS plot
    fig, ax = plt.subplots(1, 4, figsize=(FIGWIDTH,FIGHEIGHT))
    axs = ax.ravel()
    ...
    ax[2].plot(thresholds_prc, precision, color='b')
    ax[2].plot(thresholds_prc, recall, color='r')
    ax[2].invert_xaxis()
    ax[2].set(xlabel='threshold', ylabel='fraction')
    #ax[2].set_aspect('equal', 'box')
    ax[2].legend(['Precision', 'Recall'], fontsize=FONTSIZE)
    
    # Generate precision-recall Curve plot
    ax[3].plot(recall, precision, color='b')
    ax[3].plot([0, 0, 1], [1, pos_frac, pos_frac], 'r--')
    ax[3].set(xlabel='Recall', ylabel='Precision')
